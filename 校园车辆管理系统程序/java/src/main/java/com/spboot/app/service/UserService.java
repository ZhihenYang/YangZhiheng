...
public class UserService {
    ...
    @Resource
    private FangkeyonghuService fangkeyonghuService;

    @Resource
    private XiaoneirenyuanService xiaoneirenyuanService;
    ...
    public R<Object> login(String username, String pwd, String cx) {
        ...
        // 判断是否为"访客用户"角色登录时则使用这个进行登录
        if (cx.equals("访客用户")) {
            // 通过FangkeyonghuService.login 方法进行登录

            Fangwenshijian f = DB.name(Fangwenshijian.class).find(1);
            if(f.getKaishi().compareTo(Info.date("HH:mm")) > 0 || f.getJiezhi().compareTo(Info.date("HH:mm")) < 0){
                return R.error("无法在规定时间外进行登录");
            }


            Fangkeyonghu user = fangkeyonghuService.login(username, pwd);
            // user 为null 则提示用户账号或密码错误
            if (user == null) {
                return R.error("账号或密码错误");
            }
            ...
            // 设置table 变量
            // 将bean 实体类转化为session
            session = fangkeyonghuService.toSession(user);
        }
        // 判断是否为"校内人员"角色登录时则使用这个进行登录
        if (cx.equals("校内人员")) {
            // 通过XiaoneirenyuanService.login 方法进行登录

            Xiaoneirenyuan user = xiaoneirenyuanService.login(username, pwd);
            // user 为null 则提示用户账号或密码错误
            if (user == null) {
                return R.error("账号或密码错误");
            }
            ...
            // 设置table 变量
            // 将bean 实体类转化为session
            session = xiaoneirenyuanService.toSession(user);
        }
    ...
    public R<Object> editPassword(PasswordPojo passwordPojo) {
        ...
        if (roles.equals("访客用户")) {
            return fangkeyonghuService.editPassword(SessionFactory.getId(), passwordPojo.getOldPassword(), passwordPojo.getNewPassword());
        }

        if (roles.equals("校内人员")) {
            return xiaoneirenyuanService.editPassword(SessionFactory.getId(), passwordPojo.getOldPassword(), passwordPojo.getNewPassword());
        }

...
@Service
public class FangwenshijianService {

    // 获取数据库操作类mapper
    @Resource
    private FangwenshijianMapper mapper;

    /**
     *  根据id 获取一行数据
     */
    public R<Fangwenshijian> findById(Integer id) {
     ...
     *  根据Wrapper 对象进行数据筛选
     */
    public R<List<Fangwenshijian>> selectAll(Wrapper<Fangwenshijian> query) {
    ...
    public R<List<Fangwenshijian>> selectAll() {
        QueryWrapper<Fangwenshijian> wrapper = Wrappers.query();
    ...
    public R selectAll(Map<String, Object> map) {
        ...
        QueryWrapper<Fangwenshijian> wrapper = mapToWrapper(map);
        // 设置排序
     ...
     */
    public QueryWrapper<Fangwenshijian> mapToWrapper(Map<String, Object> map) {
        // 创建 QueryWrapper 对象
        QueryWrapper<Fangwenshijian> wrapper = Wrappers.query();
        ...
        if (!StringUtil.isNullOrEmpty(map.get("kaishi"))) {
            wrapper.like("kaishi", map.get("kaishi"));
        }
        if (!StringUtil.isNullOrEmpty(map.get("jiezhi"))) {
            wrapper.like("jiezhi", map.get("jiezhi"));
        }
    ...
    public R selectPages(QueryWrapper<Fangwenshijian> wrapper, IPage page) {
     ...
     * @return 是否处理成功
     */
    public R insert(Fangwenshijian entityData, Map post) {
        // 判断是否有填写开始。
        if (StringUtil.isNullOrEmpty(entityData.getKaishi())) {
            return R.error("请填写开始");
        }

        // 判断是否有填写截止。
        if (StringUtil.isNullOrEmpty(entityData.getJiezhi())) {
            return R.error("请填写截止");
        }
     ...
     * @return 是否处理成功
     */
    public R<Object> update(Fangwenshijian entityData, Map post) {
        // 判断是否有填写开始。
        if (StringUtil.isNullOrEmpty(entityData.getKaishi())) {
            return R.error("请填写开始");
        }

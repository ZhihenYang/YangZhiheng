...
public class CheweiService {
    ...
    // 获取数据库操作类mapper
    @Resource
    private CheweiMapper mapper;

    /**
     *  根据id 获取一行数据
     */
    public R<Chewei> findById(Integer id) {
     ...
     *  根据Wrapper 对象进行数据筛选
     */
    public R<List<Chewei>> selectAll(Wrapper<Chewei> query) {
    ...
    public R<List<Chewei>> selectAll() {
        QueryWrapper<Chewei> wrapper = Wrappers.query();
    ...
    public R selectAll(Map<String, Object> map) {
        ...
        QueryWrapper<Chewei> wrapper = mapToWrapper(map);
        // 设置排序
     ...
     */
    public QueryWrapper<Chewei> mapToWrapper(Map<String, Object> map) {
        // 创建 QueryWrapper 对象
        QueryWrapper<Chewei> wrapper = Wrappers.query();
    ...
    public R selectPages(QueryWrapper<Chewei> wrapper, IPage page) {
     ...
     * @return 是否处理成功
     */
    public R insert(Chewei entityData, Map post) {
        // 判断是否有填写编号。
        if (StringUtil.isNullOrEmpty(entityData.getBianhao())) {
            return R.error("请填写编号");
        }

        // 判断是否有填写名称。
        if (StringUtil.isNullOrEmpty(entityData.getMingcheng())) {
            return R.error("请填写名称");
        }

        // 判断是否有填写区域。
        if (StringUtil.isNullOrEmpty(entityData.getQuyu())) {
            return R.error("请填写区域");
        }

        // 判断是否有填写位置。
        if (StringUtil.isNullOrEmpty(entityData.getWeizhi())) {
            return R.error("请填写位置");
        }
     ...
     * @return 是否处理成功
     */
    public R<Object> update(Chewei entityData, Map post) {
        // 判断是否有填写编号。
        if (StringUtil.isNullOrEmpty(entityData.getBianhao())) {
            return R.error("请填写编号");
        }

...
import com.spboot.app.mapper.CheliangruchangMapper;
...
@Service
public class CheliangruchangService {

    // 获取数据库操作类mapper
    @Resource
    private CheliangruchangMapper mapper;

    /**
     *  根据id 获取一行数据
     */
    public R<Cheliangruchang> findById(Integer id) {
     ...
     *  根据Wrapper 对象进行数据筛选
     */
    public R<List<Cheliangruchang>> selectAll(Wrapper<Cheliangruchang> query) {
     ...
     *  直接筛选所有数据
     */
    public R<List<Cheliangruchang>> selectAll() {
        QueryWrapper<Cheliangruchang> wrapper = Wrappers.query();
    ...
    public R selectAll(Map<String, Object> map) {
        ...
        QueryWrapper<Cheliangruchang> wrapper = mapToWrapper(map);
        // 设置排序
     ...
     */
    public QueryWrapper<Cheliangruchang> mapToWrapper(Map<String, Object> map) {
        // 创建 QueryWrapper 对象
        QueryWrapper<Cheliangruchang> wrapper = Wrappers.query();
    ...
    public R selectPages(QueryWrapper<Cheliangruchang> wrapper, IPage page) {
     ...
     * @return 是否处理成功
     */
    public R insert(Cheliangruchang entityData, Map post) {
        // 判断是否有填写入场编号。
        if (StringUtil.isNullOrEmpty(entityData.getRuchangbianhao())) {
            return R.error("请填写入场编号");
        }
     ...
     * @return 是否处理成功
     */
    public R<Object> update(Cheliangruchang entityData, Map post) {
        // 判断是否有填写入场编号。
        if (StringUtil.isNullOrEmpty(entityData.getRuchangbianhao())) {
            return R.error("请填写入场编号");
        }

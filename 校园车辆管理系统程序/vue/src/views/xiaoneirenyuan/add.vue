<template>
    <div class="views-xiaoneirenyuan-add">
        <div>
            <el-card class="box-card">
                ...
                <el-form :model="form" ref="formModel" label-width="140px" status-icon validate-on-rule-change>
                    <el-form-item
                        ...
                        prop="gonghao"
                        required
                        :rules="[{required:true, message:'请填写工号'}, {validator:rule.checkRemote, message:'内容重复了', checktype:'insert', module:'xiaoneirenyuan', col:'gonghao', trigger:'blur'}]"
                    >
...
<script setup>
    ...
    import { useXiaoneirenyuanCreateForm, canXiaoneirenyuanInsert } from "@/module";
    ...
    const { form } = useXiaoneirenyuanCreateForm();
    ...
    var submit = () => {
        formModel.value.validate().then((res) => {
            ...
            loading.value = true;
            canXiaoneirenyuanInsert(form).then(
                (res) => {
                    loading.value = false;
                    if (res.code == 0) {

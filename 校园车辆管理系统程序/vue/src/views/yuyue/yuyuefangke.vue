                ...
                <el-table border :data="lists" style="width: 100%" @sort-change="sortChange" highlight-current-row>
                    ...
                    <el-table-column label="操作" fixed="right" width="220">
                        <template #default="{row}">
                            <el-button-group>
                                <el-button v-if="row.zhuangtai == '已预约'" type="primary" :icon="Plus" size="small" @click="$router.push('/admin/quxiao/add?id='+row.id)">取消预约</el-button>
...
<script setup>
    ...
    const loadList = (page) => {
        ...
        http.post("/api/yuyue/selectYuyuefangke", search).then(
            (res) => {
                loading.value = false;
                if (res.code == 0) {
                    var data = res.data;

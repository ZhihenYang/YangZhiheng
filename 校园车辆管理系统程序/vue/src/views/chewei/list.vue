                        ...
                        <template #default="{row}">
                            <el-button-group>
                                <template v-if="$session.cx == '超级管理员'">
                                    <el-button type="primary" :icon="Plus" size="small" @click="$router.push('/admin/shengyuchewei/add?id='+row.id)">添加剩余车位</el-button>

                                </template>

                                <el-button type="default" :icon="Search" size="small" @click="$router.push('/admin/shengyuchewei?cheweiid='+row.id)">查询剩余车位</el-button>
                    ...
                    <el-table-column label="操作" fixed="right" width="220">
                        <template #default="{row}">
                            <el-button-group>
                                ...
                                ><el-button type="info" :icon="InfoFilled" size="small" @click="$router.push('/admin/chewei/detail?id='+row.id)">详情</el-button>
                                </el-tooltip>
                                <el-tooltip effect="dark" content="编辑" placement="top-start"
                                ><el-button type="success" :icon="Edit" size="small" @click="$router.push('/admin/chewei/updt?id='+row.id)">编辑</el-button>
                                </el-tooltip>
...
<script setup>
    ...
    import { canCheweiSelect, useCheweiSelect, canCheweiDelete } from "@/module";
    ...
    const loadList = (page) => {
        ...
        http.post("/api/chewei/selectPages", search).then(
            (res) => {
                loading.value = false;
                if (res.code == 0) {
                    var data = res.data;

<template>
    <div class="views-zhiliuquanchu-list">
        <div>
            <el-card class="box-card">
                <template #header>
                    <div class="clearfix">
                        <span class="title"> 滞留劝出查询 </span>
                    </div>
                </template>

                <div class="form-search">
                    <el-form @submit.prevent.stop :inline="true" size="small">
                        <el-form-item label="劝出编号">
                            <el-input v-model="search.quanchubianhao"></el-input>
                        </el-form-item>
                ...
                <el-table border :data="lists" style="width: 100%" @sort-change="sortChange" highlight-current-row>
                    ...
                    <el-table-column prop="quanchubianhao" label="劝出编号" width="130">
                        <template #default="{row}"> {{ row.quanchubianhao }} </template>
                    </el-table-column>
                    ...
                    <el-table-column prop="quanchushijian" label="劝出时间" width="120">
                        <template #default="{row}"> {{ row.quanchushijian }} </template>
                    </el-table-column>
                    <el-table-column prop="zhiliushizhang" label="滞留时长">
                        <template #default="{row}"> {{ row.zhiliushizhang }} </template>
                    </el-table-column>
                    <el-table-column prop="beizhushuoming" label="备注说明">
                        <template #default="{row}"> {{ row.beizhushuoming }} </template>
                    </el-table-column>
                        ...
                        <template #default="{row}">
                            <el-button-group>
                                <el-tooltip effect="dark" content="详情" placement="top-start"
                                    ><el-button type="info" :icon="InfoFilled" size="small" @click="$router.push('/admin/zhiliuquanchu/detail?id='+row.id)">详情</el-button>
                                </el-tooltip>
                                <el-tooltip effect="dark" content="编辑" placement="top-start"
                                    ><el-button type="success" :icon="Edit" size="small" @click="$router.push('/admin/zhiliuquanchu/updt?id='+row.id)">编辑</el-button>
                                </el-tooltip>
...
<script setup>
    ...
    import { canZhiliuquanchuSelect, useZhiliuquanchuSelect, canZhiliuquanchuDelete } from "@/module";
    ...
    const deleteItems = (ids) => {
        return new Promise((resolve, reject) => {
            ElMessageBox.confirm("确定删除？")
                .then((res) => {
                    canZhiliuquanchuDelete(ids).then((res) => {
                        if (res.code == 0) {
    ...
    const loadList = (page) => {
        ...
        http.post("/api/zhiliuquanchu/selectJingshouren", search).then(
            (res) => {
                loading.value = false;
                if (res.code == 0) {
                    var data = res.data;

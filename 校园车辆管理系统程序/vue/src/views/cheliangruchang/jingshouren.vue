                ...
                <el-table border :data="lists" style="width: 100%" @sort-change="sortChange" highlight-current-row>
                    ...
                    <el-table-column label="操作" fixed="right" width="220">
                        <template #default="{row}">
                            <el-button-group>
                                <el-button v-if="row.zhuangtai == '已入场'" type="primary" :icon="Plus" size="small" @click="$router.push('/admin/chelianglichang/add?id='+row.id)">车辆离场</el-button>
...
<script setup>
    ...
    const loadList = (page) => {
        ...
        http.post("/api/cheliangruchang/selectJingshouren", search).then(
            (res) => {
                loading.value = false;
                if (res.code == 0) {
                    var data = res.data;

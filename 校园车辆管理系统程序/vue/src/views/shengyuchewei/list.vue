<template>
    <div class="views-shengyuchewei-list">
        <div>
            <el-card class="box-card">
                ...
                <el-table border :data="lists" style="width: 100%" @sort-change="sortChange" highlight-current-row>
                    ...
                    <el-table-column prop="shengyuchewei" label="剩余车位" width="120">
                        <template #default="{row}"> {{ row.shengyuchewei }} </template>
                    </el-table-column>
                    <el-table-column prop="shuoming" label="说明" width="130">
                    ...
                    <el-table-column label="操作" fixed="right" width="220">
                        <template #default="{row}">
                            <el-button-group>
                                <template v-if="$session.cx == '访客用户'">
                                    <el-button type="primary" :icon="Plus" size="small" @click="$router.push('/admin/yuyue/add?id='+row.id)">添加预约</el-button>

                                </template>


                                <el-tooltip effect="dark" content="详情" placement="top-start"
                                    ><el-button type="info" :icon="InfoFilled" size="small" @click="$router.push('/admin/shengyuchewei/detail?id='+row.id)">详情</el-button>
                                </el-tooltip>
                                <template v-if="$session.cx == '超级管理员'">
                                    <el-tooltip effect="dark" content="编辑" placement="top-start"
                                    ><el-button type="success" :icon="Edit" size="small" @click="$router.push('/admin/shengyuchewei/updt?id='+row.id)">编辑</el-button>
                                    </el-tooltip>
...
<script setup>
    ...
    import { canShengyucheweiSelect, useShengyucheweiSelect, canShengyucheweiDelete } from "@/module";
    ...
    const deleteItems = (ids) => {
        return new Promise((resolve, reject) => {
            ElMessageBox.confirm("确定删除？")
                .then((res) => {
                    canShengyucheweiDelete(ids).then((res) => {
                        if (res.code == 0) {
    ...
    const loadList = (page) => {
        ...
        http.post("/api/shengyuchewei/selectPages", search).then(
            (res) => {
                loading.value = false;
                if (res.code == 0) {
                    var data = res.data;

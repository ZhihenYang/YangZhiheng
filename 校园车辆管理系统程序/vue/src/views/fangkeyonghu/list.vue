<template>
    <div class="views-fangkeyonghu-list">
        <div>
            <el-card class="box-card">
                <template #header>
                    <div class="clearfix">
                        <span class="title"> 访客用户查询 </span>
                    </div>
                </template>
                ...
                <el-table border :data="lists" style="width: 100%" @sort-change="sortChange" highlight-current-row>
                    ...
                    <el-table-column prop="xingbie" label="性别" width="120">
                        <template #default="{row}"> {{ row.xingbie }} </template>
                    </el-table-column>
                    <el-table-column prop="lianxifangshi" label="联系方式" width="130">
                        <template #default="{row}"> {{ row.lianxifangshi }} </template>
                    </el-table-column>
                    <el-table-column prop="gerenyouxiang" label="个人邮箱" width="130">
                        <template #default="{row}"> {{ row.gerenyouxiang }} </template>
                    </el-table-column>
                    <el-table-column prop="gerenjianjie" label="个人简介">
                        <template #default="{row}"> {{ row.gerenjianjie }} </template>
                    </el-table-column>
                        ...
                        <template #default="{row}">
                            <el-button-group>
                                <el-tooltip effect="dark" content="详情" placement="top-start"
                                    ><el-button type="info" :icon="InfoFilled" size="small" @click="$router.push('/admin/fangkeyonghu/detail?id='+row.id)">详情</el-button>
                                </el-tooltip>
                                <el-tooltip effect="dark" content="编辑" placement="top-start"
                                    ><el-button type="success" :icon="Edit" size="small" @click="$router.push('/admin/fangkeyonghu/updt?id='+row.id)">编辑</el-button>
                                </el-tooltip>
...
<script setup>
    ...
    import { canFangkeyonghuSelect, useFangkeyonghuSelect, canFangkeyonghuDelete } from "@/module";
    ...
    const deleteItems = (ids) => {
        return new Promise((resolve, reject) => {
            ElMessageBox.confirm("确定删除？")
                .then((res) => {
                    canFangkeyonghuDelete(ids).then((res) => {
                        if (res.code == 0) {
    ...
    const loadList = (page) => {
        ...
        http.post("/api/fangkeyonghu/selectPages", search).then(
            (res) => {
                loading.value = false;
                if (res.code == 0) {
                    var data = res.data;

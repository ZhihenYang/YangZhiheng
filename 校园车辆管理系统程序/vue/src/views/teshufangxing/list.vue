                        ...
                        <template #default="{row}">
                            <el-button-group>
                                ...
                                <template v-if="$session.cx == '超级管理员'">
                                    ...
                                    ><el-button type="success" :icon="Edit" size="small" @click="$router.push('/admin/teshufangxing/updt?id='+row.id)">编辑</el-button>
                                    </el-tooltip>
...
<script setup>
    ...
    const loadList = (page) => {
        ...
        http.post("/api/teshufangxing/selectPages", search).then(
            (res) => {
                loading.value = false;
                if (res.code == 0) {
                    var data = res.data;

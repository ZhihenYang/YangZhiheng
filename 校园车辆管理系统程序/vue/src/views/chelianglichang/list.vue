<template>
    <div class="views-chelianglichang-list">
        <div>
            <el-card class="box-card">
                ...
                <el-table border :data="lists" style="width: 100%" @sort-change="sortChange" highlight-current-row>
                    ...
                    <el-table-column prop="lichangshijian" label="离场时间" width="120">
                        <template #default="{row}"> {{ row.lichangshijian }} </template>
                    </el-table-column>
                    <el-table-column prop="tingcheshichang" label="停车时长">
                        <template #default="{row}"> {{ row.tingcheshichang }} </template>
                    </el-table-column>
                    <el-table-column prop="lichangbeizhu" label="离场备注">
                        <template #default="{row}"> {{ row.lichangbeizhu }} </template>
                    </el-table-column>
                        ...
                        <template #default="{row}">
                            <el-button-group>
                                <el-tooltip effect="dark" content="详情" placement="top-start"
                                    ><el-button type="info" :icon="InfoFilled" size="small" @click="$router.push('/admin/chelianglichang/detail?id='+row.id)">详情</el-button>
                                </el-tooltip>
                                <el-tooltip effect="dark" content="编辑" placement="top-start"
                                    ><el-button type="success" :icon="Edit" size="small" @click="$router.push('/admin/chelianglichang/updt?id='+row.id)">编辑</el-button>
                                </el-tooltip>
...
<script setup>
    ...
    import { canChelianglichangSelect, useChelianglichangSelect, canChelianglichangDelete } from "@/module";
    ...
    const deleteItems = (ids) => {
        return new Promise((resolve, reject) => {
            ElMessageBox.confirm("确定删除？")
                .then((res) => {
                    canChelianglichangDelete(ids).then((res) => {
                        if (res.code == 0) {
    ...
    const loadList = (page) => {
        ...
        http.post("/api/chelianglichang/selectPages", search).then(
            (res) => {
                loading.value = false;
                if (res.code == 0) {
                    var data = res.data;
